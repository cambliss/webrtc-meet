...
export async function POST(req: Request, { params }: WorkspaceRouteParams) {
  ...
  const absoluteDir = path.join(process.cwd(), "public", "uploads", "workspaces", workspaceId);
  await mkdir(absoluteDir, { recursive: true });
  await writeFile(path.join(absoluteDir, fileName), bytes);

  const logoUrl = `/uploads/workspaces/${workspaceId}/${fileName}`;

  const pool = getDbPool();
  await pool.query(`UPDATE workspaces SET logo_url = $2 WHERE id = $1`, [workspaceId, logoUrl]);
