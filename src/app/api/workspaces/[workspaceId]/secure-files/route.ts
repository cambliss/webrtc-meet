...
async function runOptionalMalwareScan(params: {
  ...
  const webhookUrl = process.env.FILE_SCAN_WEBHOOK_URL?.trim();
  if (!webhookUrl) {
    return { clean: true };
  }

  // Validate that the URL is a proper http/https endpoint before making any
  // network request.  A misconfigured env variable must not block uploads.
  ...
  const failClosed =
    String(process.env.FILE_SCAN_FAIL_CLOSED || "false").toLowerCase() === "true";
  const includeContent =
    String(process.env.FILE_SCAN_INCLUDE_CONTENT || "false").toLowerCase() === "true";
  const timeoutMs = Math.max(1000, Number(process.env.FILE_SCAN_TIMEOUT_MS || "8000"));
  ...
  try {
    ...
    // Network failure, DNS error, or timeout — infrastructure errors should not
    // block uploads.  Files are only rejected when the scan service is reachable
    // and explicitly returns clean=false.
    return { clean: true };
  }
}
